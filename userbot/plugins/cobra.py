...
# 🇨 🇭 🇦 🇱 🇴      🇸 🇮 🇷    🇵 🇱 🇪 🇦 🇸 🇪    🇬 🇪 🇹 🇴 🇺 🇹    
    ...
    async def opner(event):
            if event.query.user_id == bot.uid :
                ...
                await event.edit("`>>>\n\nReopened The Main Menu of \n© Lucif3rHun's UserBot` ", buttons=dc)
            else:
...
#  🇦 🇷 🇪      🇧 🇸 🇩 🇰      🇮 🇸 🇸 🇪    🇰 🇦 🇳 🇬  🇲 🇦 🇹   🇰 🇷    🇷 🇪   🇲 🇨 
                    ...
                    try:
                        ...
                        await event.edit( "`Lucif3rHun's Personal UserBot Has Successfully loaded` >>>" + str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
                    except Exception as e:
                        await event.edit("Error{}".format(shortname, str(e))+ "Lucif3rHun's Personal UserBot Has Successfully loaded" + str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
              else:
    ...
    async def on_plug_in_callback_query_handler(event):
              if event.query.user_id == bot.uid :
                    ...
                    try:
                        ...
                        await event.edit( "`Your Lucif3rHun's Personal UserBot Has Successfully unloaded` >>>" + str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
                    except Exception as e:
                        await event.edit("Error{}".format(shortname, str(e)) +"Lucif3rHun's Personal UserBot Has Successfully unloaded"+ str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
              else:
                    shortname = event.data_match.group(1).decode("UTF-8")
                    fcix = [custom.Button.inline("  𝕭𝖆𝖈𝖐 ",data="back({})".format(shivam)),custom.Button.inline(" 𝕮𝖑𝖔𝖘𝖊 ", data="close"),custom.Button.inline(" 𝕷𝖔𝖆𝖉 ",data="load({})".format(shivam_sh1vam))]
                    reply_pop_up_alert = "Please get your own Userbot."
    ...
    async def on_plug_in_callback_query_handler(event):
            ...
            if event.query.user_id == bot.uid :
                try:
                    ...
                    await event.edit("`>>> Here Is The Main Menu of\n\n©Lucif3rHun's Personal UserBot`", buttons=buttons)
                except:
                    buttons = paginate_help(0, CMD_HELP, "helpme")
                    await event.edit("`>>> Here Is The Main Menu Of\n\n© Lucif3rHun's UserBot`", buttons=buttons)
            else:
                reply_pop_up_alert = "Please get your own Userbot."
                await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
